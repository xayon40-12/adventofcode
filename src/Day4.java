public class Day4 {
    //count the number of occurrences of regex letter in s
    public static int count(String s, String regex){
        int sum = 0;
        for(int i = 0;i<s.length();i++){//for each letter in the word s increment sum if the letter is equal to regex
            if(s.substring(i, i+1).equals(regex)) sum++;
        }
        return sum;
    }

    //return true if s1 and s2 are anagram (they share the same letters)
    public static boolean anagram(String s1, String s2){
        if(s1.length() != s2.length()) return false;//if the length is different they can't be anagram
        for(int i = 0;i<s1.length();i++){//for each letter in s1 count the number of occurrence of this letter
            // in s1 and s2 and compare these numbers
            // all these count are be equal if s1 and s2 are anagram, else they aren't so return false
            String regex = s1.substring(i, i+1);//store the actual letter to count
            //if a letter appear more than once this algorithm will count the same thing each time so it is redundant
            //but it work well enough for here so no need to optimize
            if(count(s1, regex) != count(s2,regex)) return false;
        }
        return true;
    }

    //return true if the passphrase is valid : if a word doesn't appear more than once
    public static boolean valid(String passphrase){
        String words[] = passphrase.split(" ");//split the passphrase at each space to have an array of words
        for(int i = 0;i<words.length;i++){//for each word check if there is another same word
            for(int j = i+1;j<words.length;j++){//start at i+1 because equality is symetric, so before i+1 it was already checked
                if(words[j].equals(words[i])) return false;
            }
        }
        return true;
    }

    public static boolean valid2(String passphrase){
        //same than valid but here the test is not simply equality but anagram
        String words[] = passphrase.split(" ");
        for(int i = 0;i<words.length;i++){
            for(int j = i+1;j<words.length;j++){
                if(anagram(words[j], words[i])) return false;
            }
        }
        return true;
    }
    ...
    public static void main(String args[]){
        String tab[] = input();//store the input in an array
        int sum = 0;
        for(String s: tab){//increment the sum for each passphrase valid for condition 1
            if(valid(s)) sum++;
        }
        ...
        int sum2 = 0;
        for(String s: tab){//increment the sum for each passphrase valid for condition 2
            if(valid2(s)) sum2++;
        }
    ...
    //return an array of passphrase by splitting the input at each newline
    public static String[] input(){
        String s = "oaoe rxeq vssdqtu xrk cjv yaoqp loo\n" +
